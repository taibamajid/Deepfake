...
- [Research Methodology using Deep Neural Network (DNN)](#research-methodology-using-deep-neural-network-dnn)
- [Research Methodology using Convolutional Neural Network (CNN)](#research-methodology-using-convolutional-neural-network-cnn) 
...
"In audio deepfake detection using Deep Neural Networks (DNN), the initial phase involves collecting a diverse dataset encompassing both genuine and deepfake audio samples. Preprocessing includes noise reduction, volume normalization, and standardizing the sample rate for consistency. Segmentation and framing break the audio into smaller, manageable frames. Features like Mel-frequency cepstral coefficients (MFCCs), spectral contrast, and zero-crossing rate are then extracted from these frames. Subsequently, the three sub-datasets of the FoR dataset, namely 'for-norm,' 'for-2-second,' and 'for-rerecording,' are divided into training, validation, and testing subsets. These preprocessed features serve as input to a carefully designed DNN architecture, which consists of input layers for each feature type, followed by multiple hidden layers with 128 neurons using ReLU activation functions. The model is trained on the training set, with hyperparameters fine-tuned on the validation set. Evaluation metrics like accuracy and precision gauge its performance. Post-processing. Finally, the trained DNN is deployed for audio deepfake detection.
